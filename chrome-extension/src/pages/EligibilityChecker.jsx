...
function recommendationText(tariff, reasons = [], warnings = []) {
  return [
    `Recommendation: ${tariff.name}`,
    `Why: ${reasons.length ? reasons.join('; ') : tariff.description}`,
    warnings.length ? `Caveats: ${warnings.join('; ')}` : null,
    tariff.refPath ? `Next page to check: ${tariff.refPath}` : null,
  ].filter(Boolean).join('\n');
}
    ...
    <div className="octopus-card-bg rounded-2xl p-6 space-y-5">
      ...
      {step > 0 && (
        <div className="flex items-center gap-2 text-xs text-gray-300">
          ...
          {step > 1 && <><span>›</span><span className="text-gray-300">Step {step - 1} of {questions.length}</span></>}
      ...
      {curQ && (
        <div>
          ...
          <button onClick={back} className="mt-4 text-xs text-gray-300 hover:text-gray-300 transition-colors">← Back</button>
        </div>
      )}
        ...
        <div>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <CopyButton
              ...
              value={[
                `Recommendation: ${tariffObj?.name || 'Selected tariff'}`,
                `Outcome: ${result.eligible ? 'Eligible' : 'Not eligible'}`,
                `Why: ${result.reason}`,
                result.warning ? `Caveat: ${result.warning}` : null,
                result.tip ? `Next check: ${result.tip}` : null,
              ].filter(Boolean).join('\n')}
            />
            <button onClick={reset} className="text-xs text-gray-300 hover:text-gray-300 transition-colors ml-auto">
              Start over
            </button>
          </div>
        </div>
...
export default function EligibilityChecker() {
  ...
  async function handleSubmit(e) {
    ...
    // Optional postcode lookup — validate format before calling API
    const UK_POSTCODE_RE = /^[A-Z]{1,2}[0-9][0-9A-Z]?\s?[0-9][A-Z]{2}$/i;
    if (form.postcode.trim() && UK_POSTCODE_RE.test(form.postcode.trim())) {
        ...
        <div className="mt-8 space-y-6">
          ...
          {(regionLoading || regionResult) && (
            <div className="octopus-card-bg rounded-xl p-4 text-sm">
              {regionLoading && <p className="text-gray-300">Looking up region…</p>}
              {regionResult && !regionLoading && (
                <div className="flex items-center gap-4">
                  <div>
                    ...
                    <p className="text-gray-300 text-xs">GSP: {regionResult.info.gsp} · {regionResult.info.dno}</p>
                  </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {tariff.refPath && (
                        <div className="flex flex-col items-end gap-2">
                          <Link to={tariff.refPath} className="text-xs text-gray-300 hover:text-white whitespace-nowrap underline">
                            Full guide →
                          </Link>
                          <CopyButton
                            label="Copy recommendation"
                            value={recommendationText(tariff, reasons, warnings)}
          ...
          {ineligible.length > 0 && (
            <div>
              <p className="text-sm font-semibold uppercase tracking-wider text-gray-300 mb-3">
              ...
              <div className="space-y-2">
                {ineligible.map(({ tariff, reasons }) => (
                  <div key={tariff.id} className="bg-white/5 rounded-xl p-3 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-gray-300 font-medium text-sm">{tariff.name}</p>
                      <p className="text-gray-300 text-xs mt-0.5">{reasons[0]}</p>
                    </div>
                    {tariff.refPath && (
                      <Link to={tariff.refPath} className="text-xs text-gray-300 hover:text-gray-300 underline whitespace-nowrap">
                        guide
                      </Link>
                    )}
                  </div>
