...
function solarTier(avgHours) {
  ...
  return               { label: 'Lower solar period',            colour: 'text-gray-300',  tip: 'Consider seasonal variation — total sunshine hours will be higher in spring/summer.' };
}
      ...
      <div className="octopus-card-bg rounded-2xl p-6 md:p-8">
        ...
        {activeTab === 'live' && (
          <div className="space-y-6">
            ...
            {loading && !data && (
              <p className="text-gray-300 text-sm">Loading rates…</p>
            )}
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-white/5 rounded-xl p-5 text-center">
                    ...
                    <p className="text-gray-300 text-xs mt-1">per kWh exported</p>
                  </div>
                  <div className="bg-white/5 rounded-xl p-5 text-center">
                    ...
                    <p className="text-gray-300 text-xs mt-1">per day (inc. VAT)</p>
                  </div>
                  <div className="bg-white/5 rounded-xl p-5">
                    <p className="text-xs font-semibold uppercase tracking-wider text-gray-300 mb-2">Product Code</p>
                    {data.productCode ? (
                      <CopyChip value={data.productCode} />
                    ) : (
                      <p className="text-gray-300 text-sm">—</p>
                    )}
                    <p className="text-gray-300 text-xs mt-2">Region {data.region}</p>
                  </div>
                </div>
                  ...
                  return (
                    <div className="bg-yellow-900/20 border border-yellow-500/20 rounded-xl p-4 text-sm text-gray-300">
                      <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-gray-300 text-right">Sheffield Solar PV_Live<br/>updated every 30 min</p>
                      </div>
                    </div>
                  );
                })()}
                ...
                <div className="bg-white/5 rounded-xl p-4">
                  ...
                  {!weatherData && !weatherLoading && !weatherError && (
                    <p className="text-gray-300 text-xs">Enter the customer's postcode to see their local sunshine data — useful for estimating export potential.</p>
                  )}
                  ...
                  {weatherData && (() => {
                    ...
                    return (
                      <div className="space-y-3">
                        <p className="text-xs text-gray-300">{location.district} — last 30 days</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white/5 rounded-xl p-4">
                ...
                <ul className="space-y-1.5 text-sm">
                  ...
                  <li className="flex gap-2"><span className="text-gray-300">—</span>Battery storage not required (can have battery)</li>
                </ul>
              </div>
              <div className="bg-white/5 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
                {[
                  ...
                  <div key={i} className={`rounded-lg p-2 text-center ${item.ok ? 'bg-teal-900/30 border border-teal-500/20' : 'bg-red-900/20 border border-red-500/20'}`}>
                    ...
                    {item.note && <p className="text-gray-300 text-xs mt-0.5">{item.note}</p>}
                  </div>
                ))}
              </div>
            </div>
