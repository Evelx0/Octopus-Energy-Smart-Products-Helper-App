      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
        {Object.entries(TIER).map(([key, t]) => (
          <div key={key} className={`${t.bg} border ${t.border} rounded-xl p-3`}>
            ...
            <p className="text-gray-300 text-xs leading-relaxed">{t.staffTip}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
            !activeMake ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-300 hover:text-white hover:bg-white/10'
          }`}
        >
          All makes
        </button>
        {ALL_MAKES.map(make => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
              activeMake === make ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-300 hover:text-white hover:bg-white/10'
            }`}
          >
            {make}
          </button>
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-gray-300 text-xs">({vehicles.length})</span>
      </div>
...
function VehicleCard({ vehicle: v, tier }) {
  ...
  return (
    <div className={`octopus-card-bg rounded-xl p-4 border ${t.border}`}>
      ...
      <p className="text-gray-300 text-xs leading-relaxed mt-2">{v.notes}</p>
    </div>
  );
}
