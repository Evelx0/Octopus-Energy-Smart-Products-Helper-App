...
const SEARCH_INDEX = [
  ...
  { id: 'deep-link-builder', title: 'Deep-link Builder',       category: 'Tools',     text: 'build extension links postcode region agile tracker bill simulator', route: '/deep-link-builder' },
  { id: 'tariff-fit-matrix', title: 'Tariff Fit Matrix',       category: 'Tools',     text: 'smart tariff fit matrix EV solar battery heat pump flexible usage', route: '/tariff-fit-matrix' },
  ...
  { id: 'knowledge-base', title: 'Knowledge Base',             category: 'Reference', text: 'searchable FAQ scripts agile tracker ocpp glossary decision framework', route: '/knowledge-base' },
  { id: 'release-notes', title: 'Release Notes',               category: 'Reference', text: 'what changed extension version release notes changelog', route: '/release-notes' },
  { id: 'system-health', title: 'System Health',               category: 'Tools',     text: 'health backend latency version cache service worker feature flags', route: '/system-health' },
...
const CATEGORY_COLORS = {
  ...
  'Reference':  'text-gray-300 bg-gray-400/10',
  'Page':       'text-gray-300 bg-gray-400/10',
};

function categoryColor(cat) {
  return CATEGORY_COLORS[cat] || 'text-gray-300 bg-gray-400/10';
}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10">
          ...
          />
          <kbd className="text-xs text-gray-300 bg-white/5 px-1.5 py-0.5 rounded border border-white/10 flex-shrink-0">Esc</kbd>
        </div>
              ...
              <li key={item.id}>
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {item.tab && (
                      <p className="text-gray-300 text-xs truncate">Opens on "{item.tab}" tab</p>
                    )}
                  </div>
                  <svg className="w-4 h-4 text-gray-300 flex-shrink-0 ml-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {!query && (
          <div className="px-4 py-4">
            <p className="text-xs text-gray-300 uppercase tracking-wider mb-3">Quick access</p>
            ...
            <p className="text-xs text-gray-300 mt-4 text-center">↑ ↓ navigate · Enter open · Esc close</p>
          </div>
        )}
      </div>
    </div>
