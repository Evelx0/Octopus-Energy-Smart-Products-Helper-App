...
export default function TrackerChart({ electricityRates = [], gasRates = [] }) {
  ...
  const options = {
    ...
    scales: {
      x: {
        ...
        ticks: { color: theme.tickColour, maxRotation: 45, font: { size: 11 } },
      },
      y: {
        grid: { color: theme.gridColour },
        ticks: {
          color: theme.tickColour,
          font: { size: 11 },
          callback: v => `${v}p`,
        },
        title: { display: true, text: 'Unit Rate (p/kWh)', color: theme.titleColour },
      },
    },
    plugins: {
      legend: {
        display: true,
        labels: { color: theme.labelColour, usePointStyle: true, pointStyle: 'circle', padding: 16 },
      },
      tooltip: {
        callbacks: {
          title: (items) => {
  ...
  return (
    <div className="octopus-card-bg rounded-2xl p-4 md:p-6">
      <h3 className="font-semibold text-base mb-4" style={{ color: theme.labelColour }}>Daily Unit Rates — Last 14 Days</h3>
