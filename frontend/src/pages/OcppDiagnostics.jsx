              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left py-2 pr-4 text-gray-400 font-semibold">Charger</th>
                    <th className="text-left py-2 pr-4 text-gray-400 font-semibold">OCPP</th>
        ...
        <div>
              ...
              <button
                ...
                className={`px-3 py-1 text-xs font-semibold rounded-full border transition-colors ${
                  versionFilter === pill.id
                    ...
                    : (pill.cls     || 'border-white/20 text-gray-400 hover:text-white hover:border-white/40')
                }`}
              >
                {pill.label}
              </button>
          ...
          {(q || versionFilter !== 'all') && totalResults === 0 && (
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="font-medium text-gray-400">No matching payload codes found.</p>
          ...
          {filtered.map(cat => (
            <section key={cat.id} className="mb-8">
              ...
              {!q && cat.description && (
                <p className="text-gray-400 text-xs mb-3">{cat.description}</p>
              )}
                ...
                {cat.entries.map(entry => (
                  <div key={entry.code} className="octopus-card-bg rounded-xl p-4">
                    <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
                      ...
                      {q && (
                        <span className="text-xs text-gray-600 shrink-0">{cat.label}</span>
                      )}
                    </div>
