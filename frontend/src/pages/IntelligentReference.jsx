...
export default function IntelligentReference() {
  ...
  const fetchChargers = useCallback(async () => {
    const postcode = chargerPostcode.trim();
    if (!postcode) return;
      ...
      <div className="octopus-card-bg rounded-2xl p-6 md:p-8">
        ...
        {activeTab === 'smart-charging' && (
          <div className="space-y-6">
            ...
            <div className="border-t border-white/10 pt-6">
              ...
              <p className="text-gray-400 text-xs mt-3">The Charge Cap toggle is in the Octopus app under the EV/smart charging settings. Advise customers to configure this during onboarding.</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'charging-live' && (
          <div className="space-y-8">
            ...
            {/* 24hr timeline */}
            <div>
              <p className="text-gray-400 text-xs mb-3">Typical 24-hour IO Go profile (each block = 1 hour, starting midnight)</p>
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mb-3 italic">Approximate — based on DNO region boundary mapping to Carbon Intensity API regions</p>

              {ciError && <p className="text-red-400 text-sm">{ciError}</p>}
              {ciLoading && <p className="text-gray-400 text-sm">Loading regional data…</p>}
              {ciData && !ciLoading && (() => {
                ...
                const FUEL_LABEL = { wind:'text-teal-300', solar:'text-yellow-300', gas:'text-orange-300', nuclear:'text-purple-300', imports:'text-blue-300', hydro:'text-cyan-300', biomass:'text-green-300', coal:'text-red-400', other:'text-gray-400' };
                return (
                  <div className="space-y-4">
                    <p className="text-xs text-gray-400">{ciData.shortname}</p>
                    <div className="grid grid-cols-3 gap-3 text-sm">
                      <div className="bg-white/5 rounded-xl p-4 text-center">
                        ...
                        <p className="text-xs text-gray-400">gCO₂/kWh</p>
                      </div>
                      <div className="bg-white/5 rounded-xl p-4 text-center">
                        <p className="text-xs text-gray-400 mb-1">Index</p>
                      ...
                      <div className="bg-white/5 rounded-xl p-4 text-center">
                        ...
                        <p className="text-xs text-gray-400">DNO area</p>
                      </div>
                    </div>
                    {fuels.length > 0 && (
                      <div>
            ...
            <div>
              ...
              <p className="text-gray-400 text-xs mb-3">Fallback option if the customer's home charger can't complete a session tonight.</p>
              ...
              {chargerData && (
                <div className="space-y-2">
                  {chargerData.chargers.length === 0 ? (
                    <p className="text-gray-400 text-sm">No public chargers found within 2 miles.</p>
                  ) : (
                    chargerData.chargers.map((c, i) => {
                      ...
                      return (
                        <div key={i} className="bg-white/5 rounded-xl p-4">
                          ...
                          {c.address && <p className="text-gray-400 text-xs mb-1.5">{c.address}</p>}
                          <div className="flex flex-wrap gap-2">
                            {c.distanceMi != null && (
                              <span className="text-xs text-gray-400">{c.distanceMi} mi</span>
                            )}
                  ...
                  <p className="text-xs text-gray-600 mt-1">Open Charge Map — community-maintained, status may not be real-time</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'knowledge-check' && (
          <div>
            ...
            <p className="text-gray-400 text-sm mb-6">Scenario-based questions to test your IO Go knowledge. Questions shuffle each session.</p>
