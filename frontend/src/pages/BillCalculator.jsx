      ...
      <form onSubmit={handleCalculate} className="octopus-card-bg rounded-2xl p-6 md:p-8 space-y-6 mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              {REGION_LIST.map(r => (
                <option key={r.id} value={r.id} style={{ background: '#150E38' }}>
                  {r.id} — {r.name}
                </option>
              ))}
            </select>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">
              Gas — kWh/month <span className="text-gray-400 font-normal">(optional)</span>
            </label>
            <input
              type="number"
              min="0"
              ...
              className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-white text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
            <p className="text-xs text-gray-400 mt-1">Used for Tracker and SVT gas cost comparison</p>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left py-2 pr-4 text-gray-400 font-medium">Tariff</th>
                  <th className="text-right py-2 pr-4 text-gray-400 font-medium">Elec rate</th>
                  <th className="text-right py-2 pr-4 text-gray-400 font-medium">Monthly</th>
                  <th className="text-right py-2 pr-4 text-gray-400 font-medium">Annual</th>
                  <th className="text-right py-2 text-gray-400 font-medium">vs SVT</th>
                </tr>
              </thead>
                    ...
                    <tr key={i} className={row.isSvt ? 'opacity-60' : ''}>
                      ...
                      <td className="py-3 text-right">
                        {row.isSvt ? (
                          ...
                          <span className={`font-semibold text-xs ${isCheap ? 'text-teal-400' : isExp ? 'text-red-400' : 'text-gray-400'}`}>
