  ...
  return (
    <main className="max-w-7xl mx-auto p-6 md:p-8">
      ...
      <header className="my-10">
        ...
        <p className="text-xs text-gray-400 mt-2">Tip: use the <Link to="/eligibility" className="text-teal-400 hover:underline">Eligibility Checker</Link> to filter tariffs by customer profile automatically.</p>
      </header>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse min-w-[1050px]">
          <thead>
            <tr>
              <th className="sticky left-0 bg-[#150E38] text-left px-4 py-3 text-gray-400 font-semibold text-xs uppercase tracking-wider w-44 border-b border-white/10">
                Dimension
              </th>
              {TARIFFS.map(t => (
                <th key={t.id} className="px-4 py-3 text-center border-b border-white/10 min-w-[140px]">
                  ...
                  {t.refPath && (
                    <Link to={t.refPath} className="text-xs text-gray-400 hover:text-gray-300 underline">
                      Full guide →
                    </Link>
                  )}
                </th>
      ...
      <div className="mt-4 text-xs text-gray-400 italic space-y-1">
      ...
      <section className="mt-14">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse min-w-[680px]">
            <thead>
              <tr>
                <th className="text-left px-4 py-3 text-gray-400 font-semibold text-xs uppercase tracking-wider border-b border-white/10 w-72">Scenario</th>
                <th className="text-left px-4 py-3 text-gray-400 font-semibold text-xs uppercase tracking-wider border-b border-white/10 w-44">Outcome</th>
                <th className="text-left px-4 py-3 text-gray-400 font-semibold text-xs uppercase tracking-wider border-b border-white/10">Staff notes</th>
              </tr>
            </thead>
            <tbody>
              {COMPATIBILITY_ROWS.map((row, i) => (
                <tr key={row.scenario} className={i % 2 === 0 ? 'bg-white/[0.02]' : ''}>
                  ...
                  <td className="px-4 py-3 text-gray-400 border-b border-white/5 align-top text-xs leading-relaxed">{row.notes}</td>
                </tr>
              ))}
            </tbody>
          </table>
