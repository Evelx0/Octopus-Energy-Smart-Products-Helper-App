    ...
    <div className="space-y-5">
      ...
      <div className="octopus-card-bg rounded-2xl p-6">
        ...
        <form onSubmit={handleCalculate} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1">
                Total electricity used <span className="text-gray-400">(kWh)</span>
              </label>
              <input
                type="number"
                min="0.1"
          ...
          {/* Rate status line */}
          {ratesLoading && (
            <p className="text-xs text-gray-400 italic">Loading current tariff rates for Region {region}…</p>
          )}
          {ratesError && (
          ...
          {dateFrom && dateTo && (() => {
            const days = Math.round((new Date(dateTo) - new Date(dateFrom)) / 86400000) + 1;
            if (days > 366) return (
              <p key="range-note" className="text-xs text-gray-400 italic">
      ...
      {results && !loading && (
        <>
          ...
          <p className="text-sm text-gray-400">
            ...
            <span className="text-gray-400">({results.dateFrom} → {results.dateTo})</span>
          </p>

          {/* Stacked bar chart */}
          <div className="octopus-card-bg rounded-2xl p-5">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Cost comparison — sorted cheapest first</p>
          ...
          {/* Results table */}
          <div className="octopus-card-bg rounded-2xl p-5">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">Breakdown</p>
            ...
            {results.tariffs.length > 1 && (() => {
              ...
              return saving > 0.01 ? (
                <div className="mt-4 p-3 rounded-xl bg-teal-900/25 border border-teal-700/30 text-sm">
                  ...
                  <span className="text-gray-400"> over this period.</span>
                </div>
              ) : null;
            })()}
          </div>
