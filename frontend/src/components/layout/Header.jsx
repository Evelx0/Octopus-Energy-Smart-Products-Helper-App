...
const NAV_ITEMS = [
  ...
  { id: 'terminology', href: '/terminology', label: 'Terminology', type: 'navlink' },
];

export default function Header({ onOpenSearch }) {
  const isMac = typeof navigator !== 'undefined' && /Mac/i.test(navigator.platform);
  const [mobileMenuOpen,  setMobileMenuOpen]  = useState(false);
  const [openDropdown,    setOpenDropdown]    = useState(null); // stores item.id or null
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex-shrink-0">
            <Link to="/" title="Home">
              <img src="/logo.svg" alt="Octopus Energy Smart Products" className="h-8 w-auto" />
            </Link>
          </div>
          ...
          <div className="hidden md:flex md:items-center md:space-x-4">
            ...
            <button
              ...
              title="Search (Ctrl+K)"
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-gray-400 hover:text-white border border-white/10 hover:border-white/25 text-xs transition-colors"
            >
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {NAV_ITEMS.map(item => {
              if (item.type === 'dropdown') {
                return (
                  <div key={item.id}>
                    <span role="group" aria-label={item.label} className="block px-3 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-gray-500">{item.label}</span>
