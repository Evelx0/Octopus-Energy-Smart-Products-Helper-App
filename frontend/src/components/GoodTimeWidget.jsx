...
function SignalBadge({ signal, label }) {
  ...
  if (!cfg) return <span className="text-gray-500 text-xs">—</span>;
  return (
...
export default function GoodTimeWidget({ defaultRegion = 'H', expanded = false }) {
  ...
  const fetchRates = useCallback(async (rgn) => {
    ...
    setError(null);
    try {
      const data = await getAgileRates(rgn, toDateString(new Date()));
  ...
  const currentRate = rates
    ? (rates.find(r => now >= new Date(r.valid_from) && now < new Date(r.valid_to))?.value_inc_vat ?? null)
    : null;
        ...
        <>
          ...
          <div className="flex items-end gap-3 mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">p/kWh now</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <div className="flex flex-col gap-1">
              <p className="text-xs text-gray-500">vs Today's Average</p>
            ...
            <div className="flex flex-col gap-1">
              <p className="text-xs text-gray-500">vs SVT Cap ({svtCap}p)</p>
