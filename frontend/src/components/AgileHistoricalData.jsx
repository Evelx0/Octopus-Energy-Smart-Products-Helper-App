      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-baseline">
          ...
          <div className="text-right">
            ...
            <p className="text-gray-400 text-xs">{fmtTime(stats.lowest.time)}</p>
          </div>
        </div>
        <div className="flex justify-between items-baseline">
          ...
          <div className="text-right">
            ...
            <p className="text-gray-400 text-xs">{fmtTime(stats.highest.time)}</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div className="octopus-card-bg rounded-2xl p-4 md:p-6 space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <p className="text-xs text-gray-400">Slots ≥ {SPIKE_THRESHOLD}p/kWh · top 20 shown</p>
              </div>
            </div>
            {sbpData.size > 0 && (
              <p className="text-xs text-gray-400">Settlement prices published D+1 — today's data not available.</p>
            )}

            {ciLoading && <p className="text-gray-400 text-sm">Loading carbon intensity context…</p>}
            {ciError   && <p className="text-amber-400 text-sm">{ciError}</p>}

            {spikes.length === 0 && (
              <p className="text-gray-400 text-sm">No spikes above {SPIKE_THRESHOLD}p/kWh in this period.</p>
            )}
                  ...
                  return (
                    <div key={i} className={`rounded-xl p-3 text-sm ${r.value_inc_vat >= 80 ? 'bg-red-900/30 border border-red-500/30' : 'bg-amber-900/30 border border-amber-500/30'}`}>
                      <div className="flex items-baseline justify-between gap-3 mb-1">
                        ...
                        <p className="text-gray-400 text-xs">{fmtTime(r.valid_from)}</p>
                      </div>
